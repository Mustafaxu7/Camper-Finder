...
function mouseDown(e: any) {
    ...
    if (searchCircles.length >= 1) {
        console.log(searchCircles[searchCircles.length - 1].id)
        // $(searchCircles[searchCircles.length - 1].id).remove()
        $(`[id=${searchCircles[searchCircles.length - 1].id}]`).remove();
    }
...
function generateRandomVans(make: any, numVans: number) {
    ...
    for (let i = 0; i < numVans; i++) {
        ...
        vans.push(new Van(make, model, pickName, Math.round(Math.random() * 1000), colours[Math.floor(Math.random() * colours.length)], Math.floor(Math.random() * 10000), location, features[Math.random() * features.length]))
    }
